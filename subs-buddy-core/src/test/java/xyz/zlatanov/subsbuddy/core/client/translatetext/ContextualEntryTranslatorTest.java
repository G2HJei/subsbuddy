...
class ContextualEntryTranslatorTest {
	...
	void shouldTranslate(List<SubtitleEntry> linesToTranslate, List<SubtitleEntry> expectedTranslatedLines) {
		val handler = new ContextualEntryTranslator(new CapitalizingTranslationConnector());

		val translatedLines = handler.translate(linesToTranslate);
		assertEquals(expectedTranslatedLines, translatedLines);
	}
